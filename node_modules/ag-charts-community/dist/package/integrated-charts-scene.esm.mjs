...
import { createSvgElement as createSvgElement9, isDefined } from "ag-charts-core";
...
import { Logger, clamp as clamp3 } from "ag-charts-core";
...
var Gradient = class {
  ...
  toSvg(shapeBbox) {
    ...
    this.stops.forEach(({ stop: offset4, color }) => {
      const stop = createSvgElement3("stop");
...
var ConicGradient = class extends Gradient {
  ...
  createSvgGradient(_bbox) {
    return createSvgElement4("linearGradient");
  }
};
...
var LinearGradient = class extends Gradient {
  ...
  createSvgGradient(bbox) {
    const { x0, y0, x1, y1 } = this.getGradientPoints(bbox);
    const gradient = createSvgElement5("linearGradient");
...
import { createSvgElement as createSvgElement6 } from "ag-charts-core";
var RadialGradient = class extends Gradient {
  ...
  createSvgGradient(bbox) {
    ...
    const gradient = createSvgElement6("radialGradient");
...
import { Logger as Logger2, isArray as isArray2, isPlainObject as isPlainObject2 } from "ag-charts-core";
...
var BaseProperties = class {
  ...
  set(properties) {
    ...
    if (typeof properties !== "object") {
      Logger2.warn(`unable to set ${className} - expecting a properties object`);
      return this;
    }
        ...
        if (isProperties(self[propertyKey])) {
          if (self[propertyKey] instanceof PropertiesArray) {
            ...
            if (array != null) {
              ...
              Logger2.warn(`unable to set [${propertyKey}] - expecting a properties array`);
            }
          } else {
...
import { Logger as Logger4, createSvgElement as createSvgElement7 } from "ag-charts-core";
var Image2 = class {
  ...
  toSvg(bbox, pixelRatio) {
    ...
    const image = createSvgElement7("image");
...
import {
  Logger as Logger8,
  createSvgElement as createSvgElement8
} from "ag-charts-core";
  ...
  distanceSquared(x, y) {
    ...
    for (let ci = 0, pi = 0; ci < cn; ci++) {
      switch (commands[ci]) {
        ...
        case 3 /* Curve */:
          Logger7.error("Command.Curve distanceSquare not implemented");
          break;
        case 2 /* Arc */: {
          const cx = params[pi++];
          const cy = params[pi++];
...
var Pattern = class {
  ...
  toSvg() {
    ...
    const pattern = createSvgElement8("pattern");
    pattern.setAttribute("viewBox", `0 0 ${width} ${height}`);
    pattern.setAttribute("width", String(width));
    pattern.setAttribute("height", String(height));
    pattern.setAttribute("patternUnits", "userSpaceOnUse");
    const rect = createSvgElement8("rect");
    rect.setAttribute("x", "0");
    rect.setAttribute("y", "0");
    rect.setAttribute("width", String(width));
    rect.setAttribute("height", String(height));
    ...
    rect.setAttribute("fill-opacity", String(backgroundFillOpacity));
    pattern.appendChild(rect);
    const path = createSvgElement8("path");
...
__decorateClass([
  SceneChangeDetection({ convertor: (v) => clamp4(0, v, 1) })
...
var _Text = class _Text extends Shape {
  ...
  toSVG() {
    if (!this.visible || !this.text)
      return;
    const element = createSvgElement9("text");
...
import { isArray as isArray4 } from "ag-charts-core";
...
var Path = class extends Shape {
  ...
  toSVG() {
    if (!this.visible)
      return;
    const element = createSvgElement10("path");
...
var TimeInterval = class _TimeInterval {
  ...
  every(step, options) {
    ...
    if (unsafeStep !== step) {
      Logger9.warnOnce(`interval step of [${unsafeStep}] rounded to [${step}].`);
    }
...
import { Logger as Logger10, clamp as clamp6 } from "ag-charts-core";
...
import { clamp as clamp7 } from "ag-charts-core";
function filterVisibleTicks(ticks, reversed, visibleRange) {
  ...
  const vt0 = clamp7(0, Math.floor(visibleRange[0] * ticks.length), ticks.length);
  const vt1 = clamp7(0, Math.ceil(visibleRange[1] * ticks.length), ticks.length);
...
import { Logger as Logger11, clamp as clamp8, countFractionDigits, createNumberFormatter, parseNumberFormat } from "ag-charts-core";
...
function tickStep(start, end, count, minCount = 0, maxCount = Infinity) {
  if (start === end) {
    return clamp8(1, minCount, maxCount);
  } else if (count < 1) {
    return NaN;
  }
...
import { EventEmitter as EventEmitter2, Logger as Logger13, createId as createId3, downloadUrl } from "ag-charts-core";

// packages/ag-charts-community/src/scene/canvas/hdpiCanvas.ts
import { createElement, getWindow as getWindow2 } from "ag-charts-core";
...
import { Logger as Logger12, getWindow as getWindow3, isString, toArray as toArray2 } from "ag-charts-core";
...
function prepareSceneNodeHighlight(ctx) {
  const config = toArray2(getWindow3("agChartsSceneDebug"));
  const result = [];
  ...
  constructor(canvasOptions) {
    ...
    this.destroyFns.push(
      ...
      this.imageLoader.on("image-error", ({ uri }) => {
        Logger13.warnOnce(`Unable to load image ${uri}`);
      })
    );
  }
  waitingForUpdate() {
...
var RadialColumnShape = class extends Path {
  ...
  updateBeveledPath() {
    ...
    const isEmpty2 = isNumberEqual3(innerRadius, outerRadius);
