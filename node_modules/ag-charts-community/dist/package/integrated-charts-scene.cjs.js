...
var _Color = class _Color {
  ...
  constructor(r, g, b, a = 1) {
    this.r = (0, import_ag_charts_core7.clamp)(0, r || 0, 1);
    this.g = (0, import_ag_charts_core7.clamp)(0, g || 0, 1);
    this.b = (0, import_ag_charts_core7.clamp)(0, b || 0, 1);
    this.a = (0, import_ag_charts_core7.clamp)(0, a || 0, 1);
  }
  /**
  ...
  static stringToRgba(str) {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (part.indexOf("%") >= 0) {
        value = (0, import_ag_charts_core7.clamp)(0, value, 100);
        value /= 100;
      } else if (i === 3) {
        value = (0, import_ag_charts_core7.clamp)(0, value, 1);
      } else {
        value = (0, import_ag_charts_core7.clamp)(0, value, 255);
        value /= 255;
      }
      rgba.push(value);
    }
  ...
  static lighten(c, t) {
    ...
    return _Color.fromOKLCH((0, import_ag_charts_core7.clamp)(0, oklch[0] + t, 1), oklch[1], oklch[2]);
  }
  static darken(c, t) {
    const oklch = _Color.RGBtoOKLCH(c.r, c.g, c.b);
    return _Color.fromOKLCH((0, import_ag_charts_core7.clamp)(0, oklch[0] - t, 1), oklch[1], oklch[2]);
...
var Gradient = class {
  ...
  toSvg(shapeBbox) {
    ...
    this.stops.forEach(({ stop: offset4, color }) => {
...
var BaseProperties = class {
  ...
  set(properties) {
    ...
    if (typeof properties !== "object") {
      import_ag_charts_core13.Logger.warn(`unable to set ${className} - expecting a properties object`);
      return this;
    }
        ...
        if (isProperties(self[propertyKey])) {
          if (self[propertyKey] instanceof PropertiesArray) {
            ...
            if (array != null) {
              ...
              import_ag_charts_core13.Logger.warn(`unable to set [${propertyKey}] - expecting a properties array`);
            }
          } else {
            self[propertyKey].set(value);
          }
        } else if ((0, import_ag_charts_core13.isPlainObject)(value)) {
    ...
    for (const unknownKey of keys) {
      import_ag_charts_core13.Logger.warn(`unable to set [${unknownKey}] in ${className} - property is unknown`);
    }
    return this;
  }
  toJson() {
...
var PropertiesArray = class _PropertiesArray extends Array {
  ...
  set(properties) {
    if ((0, import_ag_charts_core13.isArray)(properties)) {
...
function getColorStops(baseFills, defaultColorStops, domain, fillMode = "continuous") {
  ...
  if (fills.length === 0) {
    ...
    import_ag_charts_core14.Logger.warnOnce(`[fills] must have the stops defined in ascending order`);
    return [];
  }
...
var Image2 = class {
  ...
  createCanvasImage(ctx, image, width, height) {
    ...
    if (renderedWidth < 1 || renderedHeight < 1) {
      import_ag_charts_core15.Logger.warnOnce("Image fill is too small to render, ignoring.");
      return null;
    }
  ...
  toSvg(bbox, pixelRatio) {
    ...
    const pattern = (0, import_ag_charts_core15.createSvgElement)("pattern");
    ...
    const rect = (0, import_ag_charts_core15.createSvgElement)("rect");
    ...
    const image = (0, import_ag_charts_core15.createSvgElement)("image");
...
function parseSvg(d) {
  ...
  while (i < d.length) {
    ...
    if (commandMatch == null) {
      if (!currentCommand) {
        import_ag_charts_core16.Logger.warnOnce(`Invalid SVG path, error at index ${i}: Missing command.`);
        return;
      }
...
function parseSegment(command, d, index) {
  ...
  for (const regex of params) {
    ...
    if (match != null) {
      ...
      import_ag_charts_core16.Logger.warnOnce(
...
var logTimeGap = () => {
  ...
  if (timeSinceLastLog > LONG_TIME_PERIOD_THRESHOLD) {
    ...
    import_ag_charts_core17.Logger.log(`**** ${prettyDuration}s since last log message ****`);
  }
...
var Debug = {
  create(...debugSelectors) {
    const resultFn = (...logContent) => {
      if (Debug.check(...debugSelectors)) {
        if (typeof logContent[0] === "function") {
          logContent = (0, import_ag_charts_core17.toArray)(logContent[0]());
        }
        logTimeGap();
        import_ag_charts_core17.Logger.log(...logContent);
      }
    };
    return Object.assign(resultFn, {
      ...
      group: (name, cb) => {
        if (Debug.check(...debugSelectors)) {
          return import_ag_charts_core17.Logger.logGroup(name, cb);
        }
        return cb();
      }
    });
  ...
  check(...debugSelectors) {
    ...
    const chartDebug = (0, import_ag_charts_core17.toArray)((0, import_ag_charts_core17.getWindow)("agChartsDebug"));
  ...
  distanceSquared(x, y) {
    ...
    for (let ci = 0, pi = 0; ci < cn; ci++) {
      switch (commands[ci]) {
        ...
        case 3 /* Curve */:
          import_ag_charts_core18.Logger.error("Command.Curve distanceSquare not implemented");
          break;
        case 2 /* Arc */: {
          const cx = params[pi++];
          const cy = params[pi++];
...
var PATTERNS = {
  ...
  squares(path, { width, height, pixelRatio, padding, strokeWidth }) {
    ...
    path.lineTo(align(pixelRatio, width - offset4), align(pixelRatio, offset4));
    path.lineTo(align(pixelRatio, width - offset4), align(pixelRatio, height - offset4));
    path.lineTo(align(pixelRatio, offset4), align(pixelRatio, height - offset4));
    path.closePath();
  },
...
var Pattern = class {
  ...
  createCanvasPattern(ctx, pixelRatio) {
    ...
    if (width * scale < 1 || height * scale < 1) {
      import_ag_charts_core19.Logger.warnOnce("Pattern fill is too small to render, ignoring.");
      return null;
    }
  ...
  toSvg() {
    ...
    const pattern = (0, import_ag_charts_core19.createSvgElement)("pattern");
    pattern.setAttribute("viewBox", `0 0 ${width} ${height}`);
    pattern.setAttribute("width", String(width));
    pattern.setAttribute("height", String(height));
    pattern.setAttribute("patternUnits", "userSpaceOnUse");
    const rect = (0, import_ag_charts_core19.createSvgElement)("rect");
    rect.setAttribute("x", "0");
    rect.setAttribute("y", "0");
    rect.setAttribute("width", String(width));
    rect.setAttribute("height", String(height));
    ...
    rect.setAttribute("fill-opacity", String(backgroundFillOpacity));
    pattern.appendChild(rect);
    const path = (0, import_ag_charts_core19.createSvgElement)("path");
...
function isGradientFill(fill) {
  return (0, import_ag_charts_core20.isObject)(fill) && fill.type == "gradient";
}
function isPatternFill(fill) {
  return fill !== null && (0, import_ag_charts_core20.isObject)(fill) && fill.type == "pattern";
}
function isImageFill(fill) {
  return fill !== null && (0, import_ag_charts_core20.isObject)(fill) && fill.type == "image";
}
...
var _Shape = class _Shape extends Node {
  ...
  applySvgFillAttributes(element, defs) {
    ...
    if (typeof fill === "string") {
      ...
      const id = (0, import_ag_charts_core21.generateUUID)();
...
__decorateClass([
  SceneChangeDetection({ convertor: (v) => (0, import_ag_charts_core21.clamp)(0, v, 1) })
...
var _Text = class _Text extends Shape {
  ...
  toSVG() {
    if (!this.visible || !this.text)
      return;
    const element = (0, import_ag_charts_core22.createSvgElement)("text");
...
__decorateClass([
  SceneChangeDetection({
    convertor: (value) => (0, import_ag_charts_core22.isDefined)(value) ? String(value) : value,
...
var Caption = class extends BaseProperties {
  constructor() {
    super(...arguments);
    this.id = (0, import_ag_charts_core24.createId)(this);
...
var Path = class extends Shape {
  ...
  toSVG() {
    if (!this.visible)
      return;
    const element = (0, import_ag_charts_core25.createSvgElement)("path");
...
var TimeInterval = class _TimeInterval {
  ...
  static extent(start, stop, visibleRange) {
    ...
    if (visibleRange != null) {
      const delta3 = stop.getTime() - start.getTime();
      const t0 = start.getTime();
      start = new Date(t0 + visibleRange[0] * delta3);
  ...
  rangeIndices(start, stop, { extend = false, visibleRange = [0, 1], limit }) {
    [start, stop] = _TimeInterval.extent(start, stop, visibleRange);
  ...
  every(step, options) {
    ...
    if (unsafeStep !== step) {
      import_ag_charts_core27.Logger.warnOnce(`interval step of [${unsafeStep}] rounded to [${step}].`);
    }
    ...
    const encode7 = (date) => Math.floor((this._encode(date) - offset4) / step);
    const decode7 = (encoded) => this._decode(encoded * step + offset4);
    return new _TimeInterval(unit, milliseconds * step, hierarchy, encode7, decode7, baseStep * step, rangeCallback);
  }
};
...
var second = new TimeInterval("second", 1e3, day, encode4, decode4);
...
var minute = new TimeInterval("minute", 60 * 1e3, day, encode5, decode5);
...
var hour = new TimeInterval("hour", 60 * 60 * 1e3, day, encode6, decode6);
...
function weekday(weekStart) {
  ...
  return new TimeInterval("day", 7 * 24 * 60 * 60 * 1e3, month, encode7, decode7, 7);
}
...
var _BandScale = class _BandScale extends AbstractScale {
  ...
  set padding(value) {
    value = (0, import_ag_charts_core28.clamp)(0, value, 1);
  ...
  set paddingInner(value) {
    this.invalid = true;
    this._paddingInner = (0, import_ag_charts_core28.clamp)(0, value, 1);
  }
  get paddingInner() {
    return this._paddingInner;
  }
  set paddingOuter(value) {
    this.invalid = true;
    this._paddingOuter = (0, import_ag_charts_core28.clamp)(0, value, 1);
  }
  get paddingOuter() {
  ...
  refresh() {
    ...
    this.update();
    if (this.invalid) {
      import_ag_charts_core28.Logger.warnOnce("Expected update to not invalidate scale");
    }
  }
  convert(d, _options) {
    this.refresh();
  ...
  ordinalRange(i) {
    ...
    return (0, import_ag_charts_core28.clamp)(min, inset + step * i, max);
  }
};
__decorateClass([
  Invalidating
...
function filterVisibleTicks(ticks, reversed, visibleRange) {
  ...
  const vt0 = (0, import_ag_charts_core29.clamp)(0, Math.floor(visibleRange[0] * ticks.length), ticks.length);
  const vt1 = (0, import_ag_charts_core29.clamp)(0, Math.ceil(visibleRange[1] * ticks.length), ticks.length);
...
function tickStep(start, end, count, minCount = 0, maxCount = Infinity) {
  if (start === end) {
    return (0, import_ag_charts_core30.clamp)(1, minCount, maxCount);
  } else if (count < 1) {
    return NaN;
  }
...
function tickFormat(ticks, format) {
  const options = (0, import_ag_charts_core30.parseNumberFormat)(format ?? ",f");
  ...
  const formatter = (0, import_ag_charts_core30.createNumberFormatter)(options);
...
function range(start, end, step, visibleRange) {
  ...
  const f = 10 ** (0, import_ag_charts_core30.countFractionDigits)(step);
...
var _ContinuousScale = class _ContinuousScale extends AbstractScale {
  ...
  convert(value, options) {
    ...
    const clamp10 = options?.clamp ?? this.defaultClamp;
...
__decorateClass([
  SceneChangeDetection({ convertor: (v) => (0, import_ag_charts_core31.clamp)(0, v, 1) })
...
var HdpiCanvas = class {
  constructor(options) {
    ...
    this.pixelRatio = options.pixelRatio ?? (0, import_ag_charts_core32.getWindow)("devicePixelRatio") ?? 1;
    this.element = canvasElement ?? (0, import_ag_charts_core32.createElement)("canvas");
...
var ImageLoader = class extends import_ag_charts_core33.EventEmitter {
  constructor() {
    super(...arguments);
...
function debugStats(layersManager, debugSplitTimes, ctx, renderCtxStats, extraDebugStats = {}, seriesRect = BBox.zero) {
  ...
  ].filter(import_ag_charts_core34.isString);
...
function prepareSceneNodeHighlight(ctx) {
  const config = (0, import_ag_charts_core34.toArray)((0, import_ag_charts_core34.getWindow)("agChartsSceneDebug"));
  const result = [];
...
function debugSceneNodeHighlight(ctx, debugNodes) {
  ...
  for (const [name, node] of Object.entries(debugNodes)) {
    ...
    if (!bbox) {
      import_ag_charts_core34.Logger.log(`Scene.render() - no bbox for debugged node [${name}].`);
      continue;
    }
    ctx.globalAlpha = 0.8;
    ctx.strokeStyle = "red";
...
// packages/ag-charts-community/src/scene/scene.ts
var Scene = class extends import_ag_charts_core35.EventEmitter {
  constructor(canvasOptions) {
    super();
    this.debug = Debug.create(true, "scene" /* SCENE */);
    this.id = (0, import_ag_charts_core35.createId)(this);
    ...
    this.destroyFns.push(
      ...
      this.imageLoader.on("image-error", ({ uri }) => {
        import_ag_charts_core35.Logger.warnOnce(`Unable to load image ${uri}`);
      })
    );
  }
  waitingForUpdate() {
  ...
  download(fileName, fileFormat) {
    (0, import_ag_charts_core35.downloadUrl)(this.canvas.toDataURL(fileFormat), fileName?.trim() ?? "image");
  }
...
var Arc = class extends Path {
  ...
  get fullPie() {
    return (0, import_ag_charts_core36.isNumberEqual)(normalizeAngle360(this.startAngle), normalizeAngle360(this.endAngle));
  }
  updatePath() {
...
var Line = class extends Shape {
  ...
  toSVG() {
    if (!this.visible)
      return;
    const element = (0, import_ag_charts_core37.createSvgElement)("line");
...
var RadialColumnShape = class extends Path {
  ...
  updateBeveledPath() {
    ...
    const isStackBottom = (0, import_ag_charts_core38.isNumberEqual)(innerRadius, axisInnerRadius);
    ...
    const isEmpty2 = (0, import_ag_charts_core38.isNumberEqual)(innerRadius, outerRadius);
    ...
    if (isEmpty2 && shouldConnectBottomCircle) {
      ...
      if (!(0, import_ag_charts_core38.isNumberEqual)(topIntersectionX, 0)) {
